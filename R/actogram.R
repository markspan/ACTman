...
#' @param rollingwindow.results The moving-window results data frame produced by `run_rolling_window()` (as called from `ACTman(movingwindow = TRUE, ...)`), or `NA` if no moving-window analysis was run. Only used (and required) when `i_want_EWS = TRUE`.
#' @param i_want_EWS Boolean value indicating whether early-warning-signal statistics should be overlaid as an additional plot. Requires `rollingwindow.results` to be a real (non-NA) result.
#'
#'
...
plot_actogram <- function(workdir, ACTdata.1.sub, i, plotactogram, rollingwindow.results, i_want_EWS) {
  ...
  if (i_want_EWS == TRUE) {  # ## Initialise empty matrix for timestamps and activity counts
    ...
    for (EWS_count in seq_along(plotme)) {
      ...
      for (l.plot in seq_len(l.plot_n)) {
      ...
      ## NOTE: the `1:(nrow(...) - 1)` / `1:(nrow(...))` subscripts below are
      ## flagged by seq_linter (seq_len()/seq_along() would be safer in the
      ## nrow == 0/1 edge case), left as-is rather than mechanically
      ## rewritten: this specific EWS-overlay-on-real-rolling-window-data
      ## code path (i_want_EWS = TRUE with a real, non-NA rolling window
      ## result) has no automated test coverage yet (see test-actogram.R),
      ## so a rewrite here can't be verified the same way similar fixes
      ## were elsewhere in the package. Tracked as future work alongside
      ## adding that test coverage.
